...
app.controller("questionnaireModifyController", function ($scope, $http, $mdDialog, $routeParams, $location) {
    ...
    if(typeof id == "undefined") {
        ...
        $http.get("http://localhost:3000/questionnaireAPI/" + id).success(function(response){
            $scope.actualQuestionnaire = response;
        }).error(function(err){
            $scope.error = err;
        });
    }
    ...
    $scope.showConfirm = function(ev) {
        ...
        var confirm = $mdDialog.confirm()
            ...
            .textContent('Wenn unter diesem namen schon ein Fragebogen besteht wird dieser überschireben.')
            //.ariaLabel('Save')
            .targetEvent(ev)
            .ok('Speichern')
            .cancel('Abbrechen');
